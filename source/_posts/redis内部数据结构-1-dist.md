---
title: redis内部数据结构(1)--dist
date: 2020-11-26 14:22:02
tags: redis
categories: 中间件
---
...
本文的重点在于讨论第二个层面，Redis数据结构的内部实现，以及这两个层面的数据结构之间的关系：Redis如何通过组合第二个层面的各种基础数据结构来实现第一个层面的更高层的数据结构。
...
![redis_dict_structure](redis内部数据结构(1)--dist/redis_dict_structure.png)
...
dictDelete的源码这里忽略，具体请参考`dict.c`。需要稍加注意的是：
